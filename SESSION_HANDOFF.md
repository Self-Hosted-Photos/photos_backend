...
- `photos_infra` — S2-001 through S2-004 are all unblocked and can be done immediately (Sprint 1 infra was completed 2026-04-18)
- `photos_frontend` — S1-001 (Vite + React scaffold) can start in parallel
- `photos_admin_ui` — S1-001 (Vite + React scaffold for admin panel) can start in parallel
...
- `app/domain/models/user.py` — `User`, `EmailToken`, `RefreshToken` ORM models + `UserRole`, `UserStatus`, `EmailTokenType` enums + domain methods (`approve()`, `suspend()`, `can_upload()`)
...
#### S2-001 — Admin Approval Routes ✅ (9/9 tests)

- `app/services/admin_service.py` — `AdminService` with `get_pending_users()`, `approve_user()`, `suspend_user()`, `get_stats()`
